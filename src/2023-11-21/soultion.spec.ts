...
describe("원하는 문자열 찾기: https://school.programmers.co.kr/learn/courses/30/lessons/181878", () => {
  const randStringGen = (length: number) => {
    const lowerUpperAlphabets =
      "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
    let randString = "";
    for (let i = 0; i < length; i += 1) {
      const ranNum = parseInt(
        String(Math.random() * lowerUpperAlphabets.length),
      );
  ...
  test("연속한 문자열이 같은 경우 1을 리턴한다.", () => {
    ...
    expect(
      s[5](
        "WiuhyddrDHGFraAfoKtJnvgcxAarwZhjWsPCSJAcVWFlfADCJVSFVnhUNXvaULqMISvmHfavjBMCMRRndYwHKKuVqOxoYCPTRVIlffAJPmhVHKXZPtVPxUCMZep",
        "cxAarwZhjWsPCSJAcVWFlfADCJVSFVnhUNXvaULqMISvmHfavjBMCMRRndYwHKKuVqOxoYCPTRVIlffAJPmhVHKXZPtVPxUCMZep",
      ),
    ).toBe(1);
  });
